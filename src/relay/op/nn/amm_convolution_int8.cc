...
#include "../op_common.h"
#include "amm_convolution_int8_make.h"

namespace tvm {
namespace relay {

TVM_REGISTER_NODE_TYPE(AmmConv2DInt8Attrs);

// define relationship, we simplify the expression in the current design.
bool AmmConv2DInt8Rel(const Array<Type>& types, int num_inputs, const Attrs& attrs,
    ...
    const auto* param = attrs.as<AmmConv2DInt8Attrs>();
...
// register global 
TVM_REGISTER_GLOBAL("relay.op.nn._make.amm_conv2d_int8")
.set_body_typed([]( Expr data, Expr bias, Expr centroids, Expr lut, Expr scale,
    ...
    return MakeAmmConvInt8<AmmConv2DInt8Attrs>(data, bias, centroids, lut, scale,  
                                subvec_len, output_shape, kernel_size, strides, padding, 
                                data_layout, kernel_layout, out_layout, out_dtype, "nn.amm_conv2d_int8"); 
}); 



RELAY_REGISTER_OP("nn.amm_conv2d_int8")
    .describe(R"code(AMM 2D convolution int8 layer (e.g. spatial convolution over images).
...
Expr data, Expr bias, Expr centroids, Expr lut, Expr scale,
    ...
    .set_attrs_type<AmmConv2DInt8Attrs>()
    .set_num_inputs(5)
    ...
    .set_support_level(2)
    .add_type_rel("AmmConv2DInt8", AmmConv2DInt8Rel);

...
template <typename T>
inline Expr MakeAmmLinear(Expr data, Expr bias, Expr centroids, Expr lut, Expr scale,
...
bool AmmLinearRel(const Array<Type>& types, int num_inputs, const Attrs& attrs,
...
// register global 
TVM_REGISTER_GLOBAL("relay.op.nn._make.amm_linear")
.set_body_typed([]( Expr data, Expr bias, Expr centroids, Expr lut, Expr scale,
    ...
    return MakeAmmLinear<AmmLinearAttrs>(data, bias, centroids, lut, scale,  
                                subvec_len, output_shape, out_dtype, "nn.amm_linear"); 
}); 



RELAY_REGISTER_OP("nn.amm_linear")
...
Expr data, Expr bias, Expr centroids, Expr lut, Expr scale,
    ...
    .set_support_level(2)
    .add_type_rel("AmmLiear", AmmLinearRel);
